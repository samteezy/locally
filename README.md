...
Route by how checkable the answer is, not by topic. Names, paths, line numbers and the set of files matching a pattern come back accurate and complete — that half of an inventory it does well. Ask it *how* something works and the answer is usually right but shallow: on the [HTTP-auth run](eval-runs/2026-08-29-http-auth-locally-vs-explore.md) it made zero false claims and still stopped at the repo boundary, while the frontier agent followed the call on into the SDK. What it gets confidently wrong is anything it must derive rather than read — a default value, a resolution order, an exhaustive count, a rule about what runs when. That class accounts for most of the hard errors in [`eval-runs/`](eval-runs/), including calling a FIFO cache "LRU" in two separate runs, so check those claims against source.
