...
- `src/tools/run-task.ts` — `run_task` tool: builds dir tree, runs agentic loop for writing/generation. Home to `RUN_SYSTEM_PROMPT`, the contract's write-side half. `run_task` sent **no** system message at all before it existed — `agentic-task.ts` composes `agentConfig.systemPrompt ?? baseSystemPrompt` and only `explore_task` ever passed the second, so the promise in the tool description ("it does the task that you give it and then stops") reached the caller and never the model. Two of its rules are about where output lands rather than what it says: a model answering in chat wraps code in a markdown fence, which is free to strip from a string but lands *in the file* here, because the model calls `write_file`. Stripping it afterwards would be wrong anyway — a fence is legitimate content in a Markdown file — so the prompt names it instead. The scope rules mirror `explore_task`'s in the opposite direction: that contract fences off the verdict the reader has not earned, this one fences off the redesign nobody asked for. An agent's own `systemPrompt` still replaces it, which `run-task.test.ts` pins.
...
- `plugins/locally/` — the Claude Code plugin, listed by `.claude-plugin/marketplace.json` at the repository root. It registers the MCP server with `npx -y locally-mcp`, and it ships the skill and the subagent that `docs/claude-code.md` describes by hand. `hooks/route-to-locally.mjs` is the enforcement layer. It blocks a full `Read` over 400 lines, and it blocks the same read spelled as `cat`, `less` or `more` in `Bash` — without that second gate the first is a fence with a gate-shaped hole beside it. `head` and `tail` pass, because they are bounded reads and `head -n 200` is the fallback the denial recommends; only an explicit count over the limit is denied. The shell parse fails open at every branch, so `cd x && cat y` still gets through: the gate raises the cost of the bypass and does not close it. It can also block an unfiltered `Grep` when `LOCALLY_HOOK_GREP=1`. The Grep gate is opt-in because a large read is nearly always wasteful and a broad search is often correct. Every block names `explore_task` **and** the way back to the native tool, so a wrong block costs one turn and a disconnected server does not deadlock the session. The hook also returns an `allow` decision for `explore_task` and `usage_report`, which is the only way a plugin can ship a permissions list. `run_task` writes files and runs shell commands, so the hook leaves it to the permission prompt.
...
- Test files are colocated with the code they cover as `*.test.ts` under `src/` (e.g. `src/llm/client.test.ts`). The plugin hook is the one exception: it ships as a standalone `.mjs` outside `src/`, so `plugins/locally/hooks/route-to-locally.test.ts` sits beside it. `vitest.config.ts` names `plugins/**/*.test.ts`, and `tsconfig.plugins.json` type-checks it — `npm run typecheck` runs both projects. That suite spawns the hook rather than importing it, so what it tests is the file Claude Code runs. Its cases are the escape hatches and the env switches, because a hook that wrongly *passes* a call fails silently.
