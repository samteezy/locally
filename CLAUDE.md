...
- `src/tools/resolve-path.ts` — `FileResolver`: turns a path the model wrote into the file it meant. Tries the task's own `path`, then each root, then a targeted by-name search (`findFilesNamed`). Replaced a whole-tree index capped at 20,000 paths, which in a monorepo could drop an entire subtree and report every citation into it as missing (issue #16). Returns canonical paths, so they compare equal against the paths `Read` recorded.
- `src/tools/verify-citations.ts` — re-resolves every location in an `explore_task` answer against the filesystem; annotates, never rewrites. Prefers the answer's own `<citations>` block, which makes the common case exact rather than inferred, and renders it back out as ordinary markdown so the caller never sees a tag (`<final_answer>` is accepted as the same tag — it is what FastContext-trained models emit, and one alternation is the whole cost of running one). Without a block it falls back to four forms — inline `path:line`, `path:N-M`, `| File | Line |` table rows, and prose ranges — because a model writes them all. The footer distinguishes "nothing could be parsed" from "the file does not exist" from "the line is past the end": they are different messages to the caller and used to read the same.
...
`LOCALLY_VERIFY_SYMBOLS=0` disables the `explore_task` symbol *and* file-path checks — one switch for "check server-side that the names this answer asserts exist". It is an env var rather than a config key because `LocallyConfig` is the parsed file verbatim — the per-field env fallback only exists inside `resolveAgentConfig`, and only for `baseUrl`/`model`/`apiKey` — so a config key would be unreachable from the environment. It is deliberately not a call parameter either: an answer must not be able to switch off its own fact-checking.
